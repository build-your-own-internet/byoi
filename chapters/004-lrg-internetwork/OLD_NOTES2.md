...
We're trying to get from Client to Server, and that's not working. But... that's traversing our whole internet. Let's make this a little simpler by starting with just Router1. Can Router1 ping Server? Yes? Sweet! Let's move one hop out and jump on Router3. Can Router3 ping server? It can't... 

So, let's use the tools we've explored in previous chapters to discover why! We'll start by running a `tcpdump` on router3 and examining the output there.

> root@router3:/# tcpdump -ni eth1
our tcpdump command

> 17:43:05.906765 ARP, Request who-has 3.0.0.1 tell 5.0.1.1, length 28
Here we see an ARP request asking "who has the docker router on three-net, tell router1 on five-net". But wait! We only defined our routers on `3.0.3.1` (router3) and `3.0.1.1` (router1). Why is this ARP request looking for `3.0.0.1`? That's not part of our configuration! Let's come back to this later...

> 17:43:05.906829 IP 100.1.3.1 > 5.0.1.1: ICMP echo request, id 48, seq 5, length 64
This is a ping request sent from router3 on one-hundo-net to router1 on five-net
